...
class ProfilePageTest(TestCase):
    def setUp(self):
        ...
        self.marker1 = Marker.objects.create(id=0000, title='Lion''s mane', latitude=4.4,
                                             longitude=-5.2,
    ...
    def test_edit_profile_page(self):
        ...
        self.client.get(reverse('user', args=[self.user.id]))
        ...
        response = self.client.post(reverse('edit_profile'),
                                    {'username': 'BobsPage',
                                     ...
                                     'phone': '1234567890',
                                     'address': '123 S Ave',
                                     'Choose New Image': testImagePath, 'Save Changes': True,
                                     })
    ...
    def test_see_own_posts(self):
        ...
        self.assertContains(response,
                            self.user_marker1.marker_id.title)
        # post on page

    #   test user can delete their posts from their profile page
    def test_delete_my_posts(self):
        self.client.get(reverse('edit_profile', args=[self.user].reverse()), follow=True)
        response = self.client.post(reverse('delete_marker', args=[self.user_marker1.marker_id.id]), follow=True)
        self.assertEqual(response.status_code, 200)

        response = self.client.get(reverse('user', args=[self.user.id]))
        self.assertNotContains(response, self.user_marker1.marker_id.title)
    ...
    #     self.client.post(reverse('user', args=self.user.id.__str__()))
    ...
    def test_delete_profile(self):
        self.client.get(reverse('edit_profile', args=[self.user].reverse()), follow=True)
        response = self.client.post(reverse('delete'), args=self.user)
    ...
    def test_can_view_others_posts_on_their_profiles(self):
        ...
        user_marker2 = User_Marker.objects.create(user_id=user2, marker_id=marker2,
                                                  saved_date=datetime.now())
        response = self.client.get(reverse('user', args=[user2.id]))
        self.assertContains(response, marker2.__str__())
